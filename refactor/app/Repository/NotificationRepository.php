 ...
 */
 ...
 class NotificationRepository {
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        foreach ($users as $oneUser) {
            if ($oneUser->user_type == '2' && $oneUser->status == '1' && $oneUser->id != $exclude_user_id) { // user is translator and he is not disabled
                ...
                $jobs = $this->booking->getPotentialJobIdsWithUserId($oneUser->id); // get all potential jobs of this user
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        $user_tags = $this->booking->getUserTagsStringFromArray($users);
    ...
    public function sendSessionStartRemindNotification($user, $job, $language, $due, $duration)
    {
        ...
        if ($this->booking->isNeedToSendPush($user->id)) {
            $users_array = [$user];
            $this->notification->sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $msg_text, $this->booking->isNeedToDelayPush($user->id));
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        if ($job->gender != null && $job->gender == 'female') {
            $data['job_for'][] = 'Kvinna';
        }
